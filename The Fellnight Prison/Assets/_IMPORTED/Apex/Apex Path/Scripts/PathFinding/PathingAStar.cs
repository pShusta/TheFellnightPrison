    ...
    public class PathingAStar : PathingEngineBase
    {
        ...
        protected override PathingStatus ProcessNext()
        {
            ...
            if (!_openSet.hasNext)
            {
                ...
                if (!this.goal.isWalkable(_unitProps.attributes))
                {
            ...
            current.GetVirtualNeighbours(_successorArray, _unitProps.attributes);

            //Assign costs
            var walkableCount = _successorArray.count;
            var unitProperties = this.currentRequest.requesterProperties;
            for (int i = 0; i < walkableCount; i++)
            {
                ...
                var cost = current.g + this.cellCostStrategy.GetCellCost(n, unitProperties);
        ...
        private int GetHeuristic(IPathNode n, IPathNode current)
        {
            ...
            {
                var p = portals[i];
                if (p.isWalkable(_unitAttributes))
                {
